...
export const StepStock = () => {
  ...
  // ── Contenido del VISOR: el CamViewer3D (con el render de stock/StockPreview
  //    dentro de la escena vía `stockConfig`) + el popover contextual anclado a
  //    la región pinchada. Va al slot `visorContent` de LayoutPasoVisor, que lo
  //    envuelve en el contenedor `relative` del visor (mismo patrón que Montaje).
  const visorContent = (
    <>
      <CamViewer3D
        dimensiones={{
  ...
  //    cabecera, plegado y cajón — lo aporta LayoutPasoVisor; aquí solo el
  //    contenido, igual que StepMontaje. Cada grupo de Stock es un Collapsible.
  const controlesStock = (
    <>
      {/* Resumen READ-ONLY: offset por región (refleja el picking del visor) */}
      ...
      {/* Forma del material bruto (Rectangular / Cilíndrico) */}
      <Collapsible titulo="Forma del material bruto" defaultOpen>
      ...
      <Collapsible titulo="Dimensiones de la pieza final" defaultOpen>
      ...
      <Collapsible titulo="Sobre-material por región" defaultOpen>
        {isRect ? (
      ...
      <Collapsible titulo="Material bruto resultante" defaultOpen>
